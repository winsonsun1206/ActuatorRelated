        ...
        while True:
            try:
                ...
                if task is None:  # 如果收到None，表示应该退出线程
                    ...
                    task_id = task.get('task_id', f'can0_unknown_runin_task_id_{datetime.datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")}').strip()
               ...
               # 实时读取配置
                ...
                if task.get('operation') == 'runin_test':
                    ...
                    test_slots = task.get('parameters', {})
                    task_id = task.get('task_id', f'can0_runin_{datetime.datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")}').strip()
                    ...
                    test_slots = task.get('parameters', {})
                    task_id = task.get('task_id', f'can0_unknown_calibration_task_id_{datetime.datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")}')
                    if not test_slots or len(test_slots) == 0:
                        print("No test slots provided in the task parameters.")
                        continue
                    part_numbers = [slot['part_number'] for slot in test_slots]
