               ...
               # 实时读取配置
                ...
                if task.get('operation') == 'runin_test':
                    ...
                    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as client_socket:
                        ...
                        # time.sleep(5)
                        # #"f{station_name}_can1_bus_{can_bus_id}_{serial_number}_status".strip()"
                        # key_list = [f"{station_name}_can1_bus_{can_bus_id}_{serial_number}_calibration".strip() for can_bus_id, serial_number in zip(can_msg_addresses, serial_numbers)] + [f"{station_name}_can1_bus_{can_bus_id}_{serial_number}_status".strip() for can_bus_id, serial_number in zip(can_msg_addresses, serial_numbers)]
                        ...
                        #     continue  # 跳过后续的校准步骤
                        #self.redis_handler.set_value(f"{station_name}_can1_bus_{can_msg_addresses[0]}_{serial_numbers[0]}_status".strip(), 0)  # 重置状态为0，等待校准完成的反馈
                        # time.sleep(500)
                        self.redis_handler.set_value(task_id, 0.0)
                        calibrate_motor_parameter(part_numbers, serial_numbers, can_msg_addresses) 
                        #time.sleep(150)
...
if __name__ == "__main__":
    ...
    consumer = RabbitmqCusumer(queue_name=f'runintest_queue_{station_name}_can1', server_ip='192.168.2.66', port=5672)
