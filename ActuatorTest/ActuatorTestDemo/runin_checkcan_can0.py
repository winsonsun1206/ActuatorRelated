...
class Can0ConnectivityService:
    def __init__(self, server_ip='192.168.2.47', port=5672):
        ...
        # 🌟 恢复专门回传结果的独立队列，防止消息被树莓派秒吞，确保 Ready 留存为 1
        self.result_queue_name = f"canconnect_result_queue_{self.station_name}_can0".strip()
    ...
    def _run_heartbeat_loop(self, target_ids, stop_event, task_id, redis_key):
        ...
        print(f"🌟 [{self.can_bus}] 心跳线程已启动，正在持续维持有效电机 ID {target_ids} 的长连接（防止休眠）...")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(f"✨✨ [{self.can_bus}] 物理检测完成，结果已存入独立结果队列: [{self.result_queue_name}]")
